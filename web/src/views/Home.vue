  ...
  <div>
    <swiper :options="swiperOption">
      ...
      <m-list-card icon="menu1" title="新闻资讯" :categories="newsCats">
        <!-- 通过具名插槽把子组件中绑定的category取出来 -->
        <!-- 这样就可以让父组件不通过循环就拿到子组件中循环体的某一个变量,vue2.6版本写法 -->
...
export default {
  filters:{
    date(val){
      return dayjs(val).format('MM/DD')//把日期格式化为月份/日期
    }
  },
...
.nav-icons{
  ...
  .nav-item{
    ...
    &:nth-child(4n+1){//4n+1表示序号除以4余数为1的块border-left为none
      border-left: none;
    }
    //如果想最后一个图标有边框则用下面这个样式
